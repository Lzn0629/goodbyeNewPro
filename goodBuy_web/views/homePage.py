...
def homePage(request):
    ...
    if q:
        ...
        if request.user.is_authenticated:
            shops = personalized_shop_recommendation(request=request, keyword=q, limit=80)
            wants = personalized_want_recommendation(request=request, keyword=q, limit=80)
        else:
            shops = get_hot_shops(request=request, keyword=q, limit=80)
            wants = get_hot_wants(request=request, keyword=q, limit=80)
    ...
    #篩選
    if request.user.is_authenticated:
        shops = personalized_shop_recommendation(request=request, limit=80)
        wants = personalized_want_recommendation(request=request, limit=80)

    else:
        # 未登入使用者直接看熱門
        shops = get_hot_shops(request=request, limit=80)
        wants = get_hot_wants(request=request, limit=80)
